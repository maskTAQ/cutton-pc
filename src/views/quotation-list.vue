<template>
  <div class="quotation-list-container">
    <a @click="save" id="download" download="报价单">保存到本地</a>
    <canvas id="myCanvas" width="750" height="1240"></canvas>
  </div>
</template>
<script>
import qs from "qs";
import brandSrc from "@/assets/sign_logo.png";
const data = {
  websiteName: "中棉网",
...
export default {
  ...
  mounted() {
    const { data, key } = qs.parse(window.location.hash.split("?")[1]);
    this.data = JSON.parse(data);
    this.key = JSON.parse(key);
    const ctx = this.getCtx("myCanvas");
    this.createdQuoteList({
      ctx,
      width: 750,
      padding: 20,
      scale: 1,
  ...
  methods: {
    ...
    drawTable(ctx) {
      ...
      const data = [
        ...
        [
          { v: "合计包数:", x: 10 },
          { v: g("合计包数"), x: 10 + (halfWidth / 5) * 2 },
          { v: "质量标识:", x: 10 + halfWidth },
          { v: g("质量标识"), x: 10 + (halfWidth / 5) * 7 }
        ],
        [
          { v: "合计毛重:", x: 10 },
          { v: g("合计毛重"), x: 10 + (halfWidth / 5) * 2 },
          { v: "平均回潮:", x: 10 + halfWidth },
          { v: g("平均回潮"), x: 10 + (halfWidth / 5) * 7 }
        ],
        [
          { v: "合计皮重:", x: 10 },
          { v: g("合计皮重"), x: 10 + (halfWidth / 5) * 2 },
          { v: "平均含杂:", x: 10 + halfWidth },
          { v: g("平均含杂"), x: 10 + (halfWidth / 5) * 7 }
        ],
        [
          { v: "合计净重:", x: 10 },
          { v: g("合计净重"), x: 10 + (halfWidth / 5) * 2 },
          { v: "合计公重:", x: 10 + halfWidth },
          { v: g("合计公重"), x: 10 + (halfWidth / 5) * 7 }
        ],

        [
          { v: "批检验指标:", x: 4, bold: true },
          { v: "棉包组批各指标分布", x: 10 + (halfWidth / 5) * 2 },
          { v: "批检验指标:", x: 10 + halfWidth, bold: true },
          { v: "棉包组批各指标分布", x: 10 + (halfWidth / 5) * 7 }
        ],
        [
          { v: "主体颜色级:", x: halfWidth / 2, bold: true },
          { v: "主体长度级", x: halfWidth + halfWidth / 2, bold: true }
        ],

        [
          { v: "白棉1级:", x: halfWidth / 2 },
          { v: g("白棉1级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "32mm:", x: halfWidth + halfWidth / 2 },
          { v: g("32mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "白棉2级:", x: halfWidth / 2 },
          { v: g("白棉2级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "31mm:", x: halfWidth + halfWidth / 2 },
          { v: g("31mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "白棉3级:", x: halfWidth / 2 },
          { v: g("白棉3级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "30mm:", x: halfWidth + halfWidth / 2 },
          { v: g("30mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "白棉4级:", x: halfWidth / 2 },
          { v: g("白棉4级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "29mm:", x: halfWidth + halfWidth / 2 },
          { v: g("29mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "白棉5级:", x: halfWidth / 2 },
          { v: g("白棉5级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "28mm:", x: halfWidth + halfWidth / 2 },
          { v: g("28mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "淡点污棉1级:", x: halfWidth / 2 - 15 },
          { v: g("淡点污棉1级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "27mm:", x: halfWidth + halfWidth / 2 },
          { v: g("27mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "淡点污棉2级:", x: halfWidth / 2 - 15 },
          { v: g("淡点污棉2级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "26mm:", x: halfWidth + halfWidth / 2 },
          { v: g("26mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "淡点污棉3级:", x: halfWidth / 2 - 15 },
          { v: g("淡点污棉3级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "25mm:", x: halfWidth + halfWidth / 2 },
          { v: g("25mm"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "淡黄污棉1级:", x: halfWidth / 2 - 15 },
          { v: g("淡黄污棉1级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "马克隆主体级", x: (width / 14) * 9 + 20, bold: true }
        ],
        [
          { v: "淡黄染棉2级:", x: halfWidth / 2 - 15 },
          { v: g("淡黄染棉2级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "C1:", x: halfWidth + halfWidth / 2 },
          { v: g("C1"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "淡黄染棉3级:", x: halfWidth / 2 - 15 },
          { v: g("淡黄染棉3级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "B1:", x: halfWidth + halfWidth / 2 },
          { v: g("B1"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "黄染棉1级:", x: halfWidth / 2 },
          { v: g("黄染棉1级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "A:", x: halfWidth + halfWidth / 2 },
          { v: g("A"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "黄染棉1级:", x: halfWidth / 2 },
          { v: g("黄染棉1级"), x: (halfWidth / 10) * 7 + 10 },
          { v: "B2:", x: halfWidth + halfWidth / 2 },
          { v: g("B2"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "C2:", x: halfWidth + halfWidth / 2 },
          { v: g("C2"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],

        [
          { v: "最小值", x: (halfWidth / 7) * 3 },
          { v: g("最小值"), x: (halfWidth / 10) * 7 + 10 },
          { v: "最小值", x: (width / 14) * 10 },
          { v: g("最小值"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "最大值", x: (halfWidth / 7) * 3 },
          { v: g("最大值"), x: (halfWidth / 10) * 7 + 10 },
          { v: "最大值", x: (width / 14) * 10 },
          { v: g("最大值"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "平均值", x: (halfWidth / 7) * 3 },
          { v: g("平均值"), x: (halfWidth / 10) * 7 + 10 },
          { v: "平均值", x: (width / 14) * 10 },
          { v: g("平均值"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ],
        [
          { v: "Rd(%)", x: 20, bold: true },
          { v: "平均值", x: (halfWidth / 7) * 3 },
          { v: g("平均值"), x: (halfWidth / 10) * 7 + 10 },
          { v: "+b", x: halfWidth + 20, bold: true },
          { v: "平均值", x: (width / 14) * 10 },
          { v: g("平均值"), x: halfWidth + (halfWidth / 10) * 7 + 10 }
        ]
      ];
      ...
      ctx.fillRect(
        (width / 14) * 2,
        initHeight + lineHeight * 5,
        2,
        lineHeight * 15
      );
      ctx.fillRect(
        (halfWidth / 10) * 7,
        initHeight + lineHeight * 7,
        2,
        lineHeight * 13
      );
      ctx.fillRect(
        halfWidth + (halfWidth / 10) * 7,
        initHeight + lineHeight * 6,
        2,
        lineHeight * 19
      );
      ctx.fillRect(
        (width / 14) * 2,
        initHeight + lineHeight * 21,
        2,
        lineHeight * 4
      );
      ctx.fillRect(
    ...
    createdQuoteList({ ctx, width, padding, scale, isScale }) {
      ...
      ctx.setFontSize(18);
      ctx.fillText(
        "以上数据仅供交流使用,本网不承担任何法律责任,如有疑义,请参照国家公证检验数据!",
        0,
        1160
      );
